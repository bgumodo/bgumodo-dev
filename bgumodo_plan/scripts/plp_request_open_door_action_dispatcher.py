...
class PLP_request_door_open_action_Parameters(object):
    def __init__(self):
        # self.callback = None
...
class PLP_request_door_open_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def __init__(self):
        ...
        self.action_publisher = rospy.Publisher("speech/request_open_door_cmd", String, queue_size=10)

        self.plp_params = PLP_request_door_open_action_Parameters()
        self.plp_vars = PLP_request_door_open_action_Variables()

        rospy.Subscriber("/kcl_rosplan/action_dispatch", ActionDispatch, self.dispatch_action)

        rospy.Subscriber("speech/request_open_door_res", String, self.result_updated)
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("request_door_open_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("request_door_open_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def dispatch_action(self, action):
        if not action.name == "request_door_open":
            return
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_request_door_open_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting request_door_open action dispatcher. Waiting for command")
        Dispatcher()
