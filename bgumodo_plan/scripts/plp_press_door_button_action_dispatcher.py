...
class PLP_press_door_button_action_Parameters(object):
    def __init__(self):
        # self.callback = None
...
class PLP_press_door_button_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def __init__(self):
        ...
        rospy.Subscriber("arm/press_door_button_res", String, self.result_updated)
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("press_door_button_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("press_door_button_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def dispatch_action(self, action):
        if not action.name == "press_door_button":
            return
    ...
    def update_success(self):
        ...
        self.changeKMSFact("k_button_loc", [["button", parametersDic["door"]]], KnowledgeUpdateServiceRequest.REMOVE_KNOWLEDGE)
        self.changeKMSFact("open_door", [["door", parametersDic["door"]]], KnowledgeUpdateServiceRequest.ADD_KNOWLEDGE)
        # END
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_press_door_button_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting press_door_button action dispatcher. Waiting for command")
        Dispatcher()
