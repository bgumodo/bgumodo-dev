...
class PLP_observe_coke_can_action_Parameters(object):
    def __init__(self):
        # self.callback = None
...
class PLP_observe_coke_can_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def can_location_updated(self, a_can_location):
        ...
        self.message_store.update_named("output_can_location",a_can_location, upsert=True)
        #
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("observe_coke_can_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("observe_coke_can_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def detect_success(self):
    	return not (self.plp_params.can_location is None) and (not self.plp_params.can_location.header.frame_id == "0")
        #code to detect success over self.plp_parameters / self.plp_variables

    def detect_failure(self):
    	return not (self.plp_params.can_location is None) and (self.plp_params.can_location.header.frame_id == "0")
        #code to detect failure over self.plp_parameters / self.plp_variables

    def dispatch_action(self, action):
        if not action.name == "observe_coke_can":
            return
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_observe_coke_can_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting observe_coke_can action dispatcher. Waiting for command")
        Dispatcher()
