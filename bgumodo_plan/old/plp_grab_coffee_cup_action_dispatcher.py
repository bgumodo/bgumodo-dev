...
class PLP_grab_coffee_cup_action_Parameters(object):
    def __init__(self):
        # self.callback = None
...
class PLP_grab_coffee_cup_action_Variables(object):
    def __init__(self):
...
class MoveDispatcher(object):
    ...
    def __init__(self):
        ...
        self.action_publisher = rospy.Publisher("arm/grab_cup_cmd", PoseStamped, queue_size=10)

        self.plp_params = PLP_grab_coffee_cup_action_Parameters()
        self.plp_vars = PLP_grab_coffee_cup_action_Variables()

        rospy.Subscriber("/kcl_rosplan/action_dispatch", ActionDispatch, self.dispatch_action)

        rospy.Subscriber("arm/grab_cup_res", String, self.result_updated)
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("grab_coffee_cup_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("grab_coffee_cup_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def dispatch_action(self, action):
        if not action.name == "grab_coffee_cup":
            return
        ...
        #     if pair.key == "loc":
        #         self.cup_location = self.message_store.query_named(pair.value, String._type, False)
        #         # if not saved special value in DB, use the name (check if list returned is empty)
        #         if not self.cup_location:
        #             self.cup_location = pair.value

        # get values from action outputs history
        # if not self.cup_location:
        #     self.cup_location = self.message_store.query_named("output_cup_location", String._type, False)

        if not self.plp_params.cup_location:
            query_result = self.message_store.query_named("output_cup_location", PoseStamped._type, False)
            if query_result:
                self.plp_params.cup_location = query_result[0][0]

        # rospy.loginfo("loc value: %s",self.cup_location)
        # dispatch action if got everything
        if self.check_can_dispatch():
            self.action_publisher.publish(self.plp_params.cup_location)
        else:
        	rospy.loginfo("Failed at running action: %s. Conditions not met for dispatch", action.name)


    def check_can_dispatch(self):
        if self.plp_params.cup_location:
            return True
        return False
    ...
    def update_success(self):
        ...
        self.changeKMSFact("grabbed_cup", [], KnowledgeUpdateServiceRequest.ADD_KNOWLEDGE)
        self.changeKMSFact("arm_free", [], KnowledgeUpdateServiceRequest.REMOVE_KNOWLEDGE)
        self.changeKMSFact("coffee_at", [["loc", parametersDic["curr_loc"]]], KnowledgeUpdateServiceRequest.REMOVE_KNOWLEDGE)
        self.changeKMSFact("k_cup_loc", [], KnowledgeUpdateServiceRequest.REMOVE_KNOWLEDGE)
        # END
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_grab_coffee_cup_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting grab_coffee_cup action dispatcher. Waiting for command")
        MoveDispatcher()
