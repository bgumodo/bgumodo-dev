...
class PLP_press_elevator_button_action_Parameters(object):
    def __init__(self):
        # self.callback = None
...
class PLP_press_elevator_button_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def __init__(self):
        ...
        self.action_publisher = rospy.Publisher("arm/press_elevator_button_location_cmd", PoseStamped, queue_size=10)

        self.plp_params = PLP_press_elevator_button_action_Parameters()
        self.plp_vars = PLP_press_elevator_button_action_Variables()

        rospy.Subscriber("/kcl_rosplan/action_dispatch", ActionDispatch, self.dispatch_action)

        rospy.Subscriber("arm/press_elevator_button_location_res", String, self.result_updated)
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("press_elevator_button_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("press_elevator_button_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def dispatch_action(self, action):
        if not action.name == "press_elevator_button":
            return
        ...
        # because it has an observe
        if not self.plp_params.button_location:
            query_result = self.message_store.query_named("output_button_location", PoseStamped._type, False)
            if query_result:
                self.plp_params.button_location = query_result[0][0]

        # dispatch action if got everything
        if self.check_can_dispatch():
            self.action_publisher.publish(self.plp_params.button_location)
        else:
        	rospy.loginfo("Failed at running action: %s. Conditions not met for dispatch", action.name)


    def check_can_dispatch(self):
        if self.plp_params.button_location:
            return True
        return False
    ...
    def update_success(self):
        ...
        self.changeKMSFact("k_button_loc", [["button", parametersDic["elevator"]]], KnowledgeUpdateServiceRequest.REMOVE_KNOWLEDGE)
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_press_elevator_button_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting press_elevator_button action dispatcher. Waiting for command")
        Dispatcher()
