...
class PLP_serve_cup_action_Parameters(object):
    def __init__(self):
        # self.callback = None
...
class PLP_serve_cup_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def __init__(self):
        ...
        self.action_publisher = rospy.Publisher("arm/serve_cup_cmd", String, queue_size=10)

        self.plp_params = PLP_serve_cup_action_Parameters()
        self.plp_vars = PLP_serve_cup_action_Variables()

        rospy.Subscriber("/kcl_rosplan/action_dispatch", ActionDispatch, self.dispatch_action)

        rospy.Subscriber("arm/serve_cup_res", String, self.result_updated)
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("serve_cup_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("serve_cup_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_serve_cup_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting serve_cup action dispatcher. Waiting for command")
        Dispatcher()
