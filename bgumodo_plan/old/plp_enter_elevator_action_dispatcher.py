...
class PLP_enter_elevator_action_Parameters(object):
    def __init__(self):
        # self.callback = None
    ...
    def set_target_elevator(self, a_target_elevator):
        self.target_location = a_target_location

    def set_result(self, a_result):
        self.result = a_result

class PLP_enter_elevator_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def __init__(self):
        ...
        self.action_publisher = rospy.Publisher("navigation/enter_elevator_cmd", String, queue_size=10)

        self.plp_params = PLP_enter_elevator_action_Parameters()
        self.plp_vars = PLP_enter_elevator_action_Variables()

        rospy.Subscriber("/kcl_rosplan/action_dispatch", ActionDispatch, self.dispatch_action)

        rospy.Subscriber("navigation/enter_elevator_res", String, self.result_updated)
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("enter_elevator_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("enter_elevator_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def dispatch_action(self, action):
        if not action.name == "enter_elevator":
            return
        ...
        for pair in action.parameters:
            if pair.key == "elevator":
                self.target_elevator = self.message_store.query_named(pair.value, String._type, False)
                # if not saved special value in DB, use the name (check if list returned is empty)
                if not self.target_elevator:
                    self.target_elevator = pair.value

        # get values from action outputs history
        if not self.target_elevator:
            self.target_elevator = self.message_store.query_named("output_target_elevator", String._type, False)

        # dispatch action if got everything
        if self.check_can_dispatch():
            self.action_publisher.publish(self.target_elevator)
        else:
        	rospy.loginfo("Failed at running action: %s. Conditions not met for dispatch", action.name)


    def check_can_dispatch(self):
        if not (self.target_elevator is None):
            return True
        return False
    ...
    def update_success(self):
        ...
        self.changeKMSFact("robot_in_elevator", [["elevator", parametersDic["elevator"]]], KnowledgeUpdateServiceRequest.ADD_KNOWLEDGE)
        # END
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_enter_elevator_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting enter_elevator action dispatcher. Waiting for command")
        Dispatcher()
