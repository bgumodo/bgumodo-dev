...
class PLP_observe_elevator_button_action_Parameters(object):
    def __init__(self):
        # self.callback = None
    ...
    def set_button_location(self, a_button_location):
        self.button_location = a_button_location

    def set_button(self, a_button):
        self.button = a_button

class PLP_observe_elevator_button_action_Variables(object):
    def __init__(self):
...
class Dispatcher(object):
    ...
    def __init__(self):
        ...
        self.action_publisher = rospy.Publisher("vision/observe_btn_cmd", String, queue_size=10)

        self.plp_params = PLP_observe_elevator_button_action_Parameters()
        self.plp_vars = PLP_observe_elevator_button_action_Variables()

        rospy.Subscriber("/kcl_rosplan/action_dispatch", ActionDispatch, self.dispatch_action)

        rospy.Subscriber("vision/observe_btn_res", PoseStamped, self.button_location_updated)

    def button_location_updated(self, a_button_location):
        rospy.loginfo("got button posestamped")
        self.plp_params.set_button_location(a_button_location)
        # Because output param
        self.message_store.update_named("output_button_location",a_button_location, upsert=True)
        #
    ...
    def parameters_updated(self):
        ...
        if self.detect_success():
    		rospy.loginfo("observe_elevator_button_action_dispatcher: detected success")
    		self.update_success()
    		self.reset_dispatcher()
        elif self.detect_failure():
    		rospy.loginfo("observe_elevator_button_action_dispatcher: detected failure")
    		self.update_fail()
    		self.reset_dispatcher()
    ...
    def detect_success(self):
    	return (not self.plp_params.button_location is None) and (not self.plp_params.button_location.header.frame_id == "0")
        #code to detect success over self.plp_parameters / self.plp_variables

    def detect_failure(self):
    	return (not self.plp_params.button_location is None) and (self.plp_params.button_location.header.frame_id == "0")
        #code to detect failure over self.plp_parameters / self.plp_variables

    def dispatch_action(self, action):
        if not action.name == "observe_elevator_button":
            return
        ...
        for pair in action.parameters:
            if pair.key == "elevator":
                self.button = self.message_store.query_named(pair.value, String._type, False)
                # if not saved special value in DB, use the name (check if list returned is empty)
                if not self.button:
                    self.button = pair.value

        # get values from action outputs history
        if not self.button:
            self.button = self.message_store.query_named("output_button", String._type, False)

        # dispatch action if got everything
        if self.check_can_dispatch():
            # DEFAULT
            self.action_publisher.publish(String("Go"))
        else:
    ...
    def update_success(self):
        ...
        self.changeKMSFact("k_button_loc", [["button", parametersDic["elevator"]]], KnowledgeUpdateServiceRequest.ADD_KNOWLEDGE)
        # END
...
if __name__ == '__main__':
    try:
        rospy.init_node("plp_observe_elevator_button_action_dispatcher", anonymous=False)
        rospy.loginfo("Starting observe_elevator_button action dispatcher. Waiting for command")
        Dispatcher()
